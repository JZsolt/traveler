...
import type { ScheduleEditorProps } from '@/types/editor'
import type { ScheduleItemDraft } from '@/types/hooks'

const GUIDE_KEYS = ['history', 'mustSee', 'funFacts', 'tips'] as const

export function ScheduleEditor({ draft, onChange, aiGuidePanel }: ScheduleEditorProps) {
  function set<K extends keyof ScheduleItemDraft>(field: K, value: ScheduleItemDraft[K]) {
    onChange({ ...draft, [field]: value })
  }

  function setGuide(field: typeof GUIDE_KEYS[number], value: string[]) {
