...
import type { GuideItemProps, GuideInfoProps } from '@/types/components'

const sections: Record<string, { icon: typeof Landmark; label: string; color: string }> = {
...
export function GuideInfo({ guide }: GuideInfoProps) {
  const [open, setOpen] = useState(false)

  if (!guide) return null
  const sectionEntries = Object.entries(guide).filter(
    ([k, v]) => k in sections && Array.isArray(v) && v.length > 0
  ) as [string, unknown[]][]
  if (sectionEntries.length === 0 && !guide.image) return null

  const guideImage = guide.image as { url: string; caption?: string } | undefined
      ...
      {open && (
        <div className="mt-2.5 space-y-3 bg-slate-50/80 rounded-xl p-3 md:p-4">
          {guideImage && (
            <img
              src={guideImage.url}
              alt={guideImage.caption || ''}
              className="w-full h-40 md:h-52 object-cover rounded-lg"
              loading="lazy"
            />
          )}
          {guideImage?.caption && (
            <p className="text-[10px] text-slate-400 text-center -mt-2">{guideImage.caption}</p>
          )}
                ...
                <ul className="space-y-0.5 ml-5">
                  {items.map((item, i) => (
                    <li key={i} className="text-[12px] md:text-[13px] text-slate-600 leading-[1.65] list-disc marker:text-slate-300">
                      <GuideItem item={item as string | { text: string; url?: string }} />
                    </li>
                  ))}
                </ul>
              </div>
