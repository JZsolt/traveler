...
import { isUsefulLinkArray } from '@/types/guards'
import type { UsefulLink } from '@/types/trip'
import type { UsefulLinksEditorProps, UsefulLinksProps } from '@/types/components'

const EMPTY_LINK: UsefulLink = { emoji: '🔗', name: '', desc: '', url: '' }

function UsefulLinksEditor({ items, onChange, validationError }: UsefulLinksEditorProps) {
  function updateItem(index: number, field: keyof UsefulLink, value: string) {
    const updated = [...items]
...
export function UsefulLinks({ links, trip, slug, refetch }: UsefulLinksProps) {
  const [draft, setDraft] = useState<UsefulLink[] | null>(null)
  ...
  function handleApply(suggestion: UsefulLink[]) {
  ...
  const editor = (
    <>
      ...
      />
      {showAi && (
        <AiSuggestionPanel<UsefulLink[]>
          section="usefulLinks"
          trip={trip}
          validateSuggestion={isUsefulLinkArray}
          onApply={handleApply}
          renderPreview={items => (
