...
function BookingChecklistEditor({ items, onChange, validationError }: BookingChecklistEditorProps) {
  function updateItem(index: number, field: keyof BookingChecklistItem, value: string) {
    const updated = [...items]
...
export function BookingChecklist({ items, trip, slug, refetch }: BookingChecklistProps) {
  const [draft, setDraft] = useState<BookingChecklistItem[] | null>(null)
  ...
  function handleApply(suggestion: BookingChecklistItem[]) {
  ...
  const editor = (
    <>
      ...
      />
      {showAi && (
        <AiSuggestionPanel<BookingChecklistItem[]>
          section="bookingChecklist"
          trip={trip}
          validateSuggestion={isBookingChecklistItemArray}
          onApply={handleApply}
          renderPreview={items => (
