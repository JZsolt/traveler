...
import type { ImportBatchError, ImportBatchResult, ImportSingleResult } from '../src/types/apiServer'
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (mode !== 'create' && mode !== 'upsert-by-slug') {
    return res.status(400).json(err('INVALID_MODE', 'Ervenytelen import mod. Hasznalj "create" vagy "upsert-by-slug" modot.'))
  }

  const backups = body.backups
  if (!Array.isArray(backups) || backups.length === 0) {
    return res.status(400).json(err('INVALID_BACKUPS', 'A "backups" mezonek nem-ures tombnek kell lennie.'))
  }
