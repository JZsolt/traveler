...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (!apiKey) {
    return res
      .status(500)
      .json({ error: 'GEMINI_API_KEY nincs konfigurálva a szerveren.' });
  }
  ...
  try {
    ...
    const contents = [
      ...
      })),
      {
        ...
        parts: [
          {
            text: 'A fenti beszelgetes alapjan generald a kompakt utitervet JSON-ben! A PONTOS SCHEMA-t kovesd, semmi mas szoveg, csak JSON.',
          },
        ],
      },
    ];
    ...
    return res.status(200).json({ trip: validated.data });
  } catch (err) {
    ...
    if (is429) {
      return res.status(429).json({
        error:
          'Elerved az ingyenes AI limitet. Probald ujra kesobb, vagy valts Gyors terv modra.',
        retryable: true,
      });
    }
